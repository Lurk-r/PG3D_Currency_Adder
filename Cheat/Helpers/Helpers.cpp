...
namespace Helpers
{
    ...
    void MouseFix(bool menuVisible) {
        static bool wasMenuVisible = false;
        ...
        // Handle ShowCursor reference count (game drives it negative)
        if (menuVisible) {
            // Force cursor visible immediately
            while (ShowCursor(TRUE) < 0) {}
        } else if (wasMenuVisible) {
            // Just closed: force cursor hidden
            while (ShowCursor(FALSE) >= 0) {}
        }

        // Handle cursor clipping
        if (menuVisible && !wasMenuVisible) {
            // Menu just opened - confine to window
            RECT windowRect;
            if (GetWindowRect(hwnd, &windowRect)) {
                ClipCursor(&windowRect);
            }
        } else if (!menuVisible && wasMenuVisible) {
            // Menu just closed - release confinement
            ClipCursor(nullptr);
        }

        // Continuous cursor centering when menu is closed (game behavior)
        if (!menuVisible) {
            RECT rect{};
